...
public abstract class StandaloneYamlConfigManager<P extends XiaMoJavaPlugin> extends PluginObject<P>
{
    ...
    public void reload()
    {
        ...
        if (!configFile.exists())
        {
            ...
            {
                logger.error("Can't create file '%s' to save configuration! Not reloading...".formatted(configFile));
                return;
            }
        }
        ...
        {
            logger.error("Unable to load configuration from '%s': %s".formatted(configFile, e.getMessage()));
            return;
        }
